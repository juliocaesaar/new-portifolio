...
function SantaHat() {
  return (
    <svg
      ...
      className="santa-hat-svg absolute -top-2 -left-1 w-[18px] h-[18px] rotate-[10deg] z-10"
    >
      <path
...
// Verifica se está no período de Natal (20-29 de dezembro)
function isChristmasPeriod(): boolean {
  const today = new Date();
  const month = today.getMonth(); // 0-11 (dezembro = 11)
  const day = today.getDate();
  return month === 11 && day >= 20 && day <= 29;
}
